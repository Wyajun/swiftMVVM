...
class CirCleView: UIView, UIScrollViewDelegate {
    ...
    var imageArray: [String!]! {
        ...
        */
        didSet {
            ...
            self.pageIndicator.frame = CGRectMake(self.frame.size.width - 20 * CGFloat(imageArray.count), self.frame.size.height - 20, 20 * CGFloat(imageArray.count), 20)
    ...
    private func setUpCircleView() {
        ...
        //设置计时器
         timer = JDTimer.timerWithInterval(2*NSEC_PER_SEC, leeway:0, queue:dispatch_get_main_queue() ,block:{
            [weak self] in
...
//        self.timer = NSTimer.scheduledTimerWithTimeInterval(TimeInterval, target: self, selector: "timerAction", userInfo: nil, repeats: true)
    ...
    private func setScrollViewOfImage(){
        ...
        self.currentImageView.kf_setImageWithURL(NSURL(string:  self.imageArray[self.indexOfCurrentImage])!, placeholderImage: placeholderImage)
         self.nextImageView.kf_setImageWithURL(NSURL(string: self.imageArray[self.getNextImageIndex(indexOfCurrentImage: self.indexOfCurrentImage)])!, placeholderImage: placeholderImage)
         self.lastImageView.kf_setImageWithURL(NSURL(string:  self.imageArray[self.getLastImageIndex(indexOfCurrentImage: self.indexOfCurrentImage)])!, placeholderImage: placeholderImage)
    }
